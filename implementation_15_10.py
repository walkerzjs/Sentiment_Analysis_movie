...
def define_graph(glove_embeddings_arr):
    ...
    tensors"""
    input_data = tf.placeholder(tf.int32, shape=[batch_size, max_col_size], name='input_data')
    labels = tf.placeholder(tf.float32, shape=[batch_size, 2], name='labels')
    embed = tf.Variable(tf.zeros([batch_size, max_col_size, glove_embeddings_arr.shape[1]]), dtype=tf.float32, name='embed')
    embed = tf.nn.embedding_lookup(glove_embeddings_arr, input_data, name='embed_lookup')
    
    
    with tf.variable_scope('lstm15'):
#        dropout_keep_prob = tf.placeholder(tf.float32,shape=(),name='dropout_keep_prob')
        dropout_keep_prob = tf.placeholder_with_default(1, shape=(), name='dropout_keep_prob')
...
#        preds = tf.nn.softmax(logits)
        ...
        loss = tf.reduce_mean(batch_xentropy, name='loss')
        optimizer = tf.train.AdamOptimizer(0.001, name='optimizer').minimize(loss)
        prediction_checked = tf.equal(tf.argmax(logits,1), tf.argmax(labels,1))
        accuracy = tf.reduce_mean(tf.cast(prediction_checked, tf.float32), name='accuracy')
